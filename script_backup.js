...
const EXTERNAL_LINKS_DOC_NAME = 'config/external_links'; // Path per i link esterni
...
/**
 * Mostra una modale con un'immagine esterna.
...
window.showImageModal = showImageModal; // Espone la funzione globalmente


// --- FUNZIONI DI PATH FIREBASE ---
...
// Path per il documento dei link esterni
const getExternalLinksDocRef = () => {
    return doc(db, `artifacts/${appId}/public/data/${EXTERNAL_LINKS_DOC_NAME}`);
};
    ...
    const globalAccessHtml = `
        <div class="max-w-md mx-auto p-8 bg-white rounded-xl shadow-xl border border-gray-100">
            ...
            <div class="space-y-4">
                ...
                <a href="${externalLinks.classificaUrl}" target="_blank" class="w-full text-white font-semibold py-2 px-4 rounded-lg transition duration-150 bg-indigo-500 hover:bg-indigo-600 flex items-center justify-center shadow-lg">
                    <i data-lucide="bar-chart-3" class="w-5 h-5 mr-2"></i>
                    Classifica Aggiornata
                </a>
                
                <a href="${externalLinks.risultatiUrl}" target="_blank" class="w-full text-white font-semibold py-2 px-4 rounded-lg transition duration-150 bg-teal-500 hover:bg-teal-600 flex items-center justify-center shadow-lg">
...
const renderLogin = (message = '') => {
    document.getElementById('header-title').textContent = "Accesso alla Serie SeriA";
    const loginHtml = `
        <div class="max-w-md mx-auto p-8 bg-white rounded-xl shadow-xl border border-gray-100">
            ${message ? `<div class="error-message">${message}</div>` : ''}
...
const renderHomepage = () => {
    ...
    // --- LOGICA DI ORDINAMENTO PER RUOLO ---
    
    // Mappa di ordinamento ruoli
    ...
    const sortedPlayers = currentTeam.players 
        ? [...currentTeam.players].sort((a, b) => {
            const roleA = a.role ? a.role.toUpperCase() : 'Z';
            const roleB = b.role ? b.role.toUpperCase() : 'Z';
            
            // 1. Ordina per Ruolo (usando la mappa)
            const orderA = ROLE_ORDER_MAP[roleA] || 99; // 99 come fallback
            const orderB = ROLE_ORDER_MAP[roleB] || 99;

            if (orderA !== orderB) {
                return orderA - orderB;
            }

            // 2. Ordina per Nome (alfabeticamente, se i ruoli sono uguali)
            const nameA = a.name || '';
            const nameB = b.name || '';
    ...
    // Lista dei giocatori (usa ora sortedPlayers)
    const playersListHtml = sortedPlayers.length > 0
        ? sortedPlayers.map(p => `
    ...
    if (!draftStatus.isDraftActive && !isDraftFinished) {
        ...
        if (hasDraftedThisRound) {
            ...
            if (!draftPlayers || draftPlayers.length === 0) {
                draftMessageHtml += `
                    <div class="text-center text-gray-500 p-8 bg-gray-100 rounded-lg border border-gray-200 mt-4">Nessun giocatore disponibile nel draft al momento.</div>
                `;
            } else {
...
const renderAdminTeamView = (teamId) => {
    ...
    const teamViewHtml = `
        <div class="space-y-6">
            ...
            <div class="p-6 bg-blue-50 rounded-xl border border-blue-200 shadow-lg">
                <h3 class="text-xl font-bold text-gray-800 mb-2">Dettagli Squadra</h3>
...
window.resetTurnIndex = async () => {
    ...
    const confirmation = await showConfirmationModal(`Sei sicuro di voler resettare i turni di draft al Turno 1? Questo NON cambierà il numero del Round (${draftStatus.roundNumber}).`);
    if (!confirmation) return;
    ...
    try {
        ...
        alert("Avanzamento turni resettato al Turno 1.");
    } catch (error) {
        console.error("Errore nel reset dei turni:", error);
        alert("Errore nel reset dei turni. Controlla la console.");
    }
};
...
const renderAdmin = () => {
    ...
    const adminHtml = `
        <div class="space-y-8">
            ...
            <div class="p-6 bg-indigo-50 rounded-xl border border-indigo-200 shadow-lg text-center">
                ...
                <div class="p-6 bg-white rounded-xl border border-gray-200 shadow-lg mt-8 text-left">
                    <h2 class="text-xl font-bold text-gray-800 mb-4 flex items-center">
                        <i data-lucide="link" class="w-5 h-5 mr-2 text-indigo-500"></i>
                        Gestione URL Classifica & Risultati
                    </h2>
                    <div class="space-y-4">
                        <div>
                            <label for="adminClassificaUrl" class="block text-sm font-medium text-gray-700 mb-1">URL Classifica Aggiornata</label>
                            <input type="url" id="adminClassificaUrl" class="input-style" value="${externalLinks.classificaUrl || ''}" placeholder="Inserisci URL Immagine Classifica">
                            <button onclick="showImageModal(document.getElementById('adminClassificaUrl').value, 'Anteprima Classifica')" 
                                    class="text-xs text-indigo-500 hover:text-indigo-700 mt-1">
                                Visualizza Anteprima
                            </button>
                        </div>
                        <div>
                            <label for="adminRisultatiUrl" class="block text-sm font-medium text-gray-700 mb-1">URL Risultati Giornata</label>
                            <input type="url" id="adminRisultatiUrl" class="input-style" value="${externalLinks.risultatiUrl || ''}" placeholder="Inserisci URL Immagine Risultati">
                            <button onclick="showImageModal(document.getElementById('adminRisultatiUrl').value, 'Anteprima Risultati')" 
                                    class="text-xs text-teal-500 hover:text-teal-700 mt-1">
                                Visualizza Anteprima
                            </button>
                        </div>
                        <button id="saveExternalLinksBtn" class="btn-primary w-full bg-indigo-600 hover:bg-indigo-700">
    ...
    // NUOVO LISTENER PER IL SALVATAGGIO
    document.getElementById('saveExternalLinksBtn').addEventListener('click', () => {
        const classifica = document.getElementById('adminClassificaUrl').value.trim();
        const risultati = document.getElementById('adminRisultatiUrl').value.trim();
        saveExternalLinks(classifica, risultati);
    });
};
...
const startDataListeners = () => {
    ...
    draftStatusUnsub = onSnapshot(getDraftStatusDocRef(), (docSnapshot) => {
        if (docSnapshot.exists()) {
            ...
            if (typeof draftStatus.roundNumber === 'undefined') draftStatus.roundNumber = 1; 
        } else {
    ...
    // NUOVO LISTENER: Caricamento link esterni
    externalLinksUnsub = onSnapshot(getExternalLinksDocRef(), (docSnapshot) => {
        if (docSnapshot.exists()) {
        ...
        // Ri-renderizza le schermate di accesso se necessario
        const currentTitle = document.getElementById('header-title').textContent;
        if (currentTitle.includes('Accesso alla Lega SeriA')) renderGlobalAccess();
        if (currentTitle.includes('Accesso alla Serie SeriA')) renderLogin();

        // Ri-renderizza l'Admin per aggiornare i campi URL
        if (currentTeam && currentTeam.isAdmin) renderAdmin();
    }, (error) => console.error("Errore listener link esterni:", error));
